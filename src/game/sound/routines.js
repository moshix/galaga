...
/**
 * The sound CPU's routine tables. A leaf module with no imports, so every
...
/**
 * Routine name (the label in reference/galaga-sound.asm) -> function.
...
export function soundAt(addr) {
  const fn = SOUND_AT[addr];
  if (fn === undefined) throw new Error(`sound CPU: no routine registered at $${addr.toString(16).toUpperCase()}`);
  return fn;
}
